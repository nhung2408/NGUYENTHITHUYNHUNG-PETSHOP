...
if (isset($_POST['btnThemThu'])) {
    ...
    if (isset($hinhThu)) {
        ...
        if (!preg_match('/\.(jpg|gif|png)$/i', basename($hinhThu['name']))) {
            echo ' <div class="alert alert-success">Định dạng file không phải là ảnh</div>';
            $flag = 0;
        }
    }
    if ($flag == 1) {
        $sql = "INSERT INTO `thunuoi`(`idLoai`, `tenThu`,`tuoiThu`,`canNangThu`,`gioiTinhThu`,`hinhThu`,`giaThu`, `moTaThuNuoi`) VALUES ('{$idLoai}','{$tenThu}','{$tuoiThu}','{$canNang}','{$giongThu}','{$hinhThu}','{$giaThu}','{$motaThu}')";
                            ...
                            <div class="form-group">
                                <select class="form-control" name="cmbCL" onchange="form1.submit()">
                                    ...
                                    while ($row = mysqli_fetch_array($result)) { ?>
                                        <option value="<?php echo $row['idCL']; ?>" <?php if (isset($_POST['cmbCL']) && $_POST['cmbCL'] == $row['idCL']) {
                                                                                        echo 'selected';
                                                                                    } ?>><?php echo $row['tenCL']; ?>
                                        </option>
                                    <?php } ?>
                    ...
                    <form method="post" name="form2" action="" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <select class="form-control" name="cmbLoai">
                                ...
                                $result = mysqli_query($link, $sql);
                                while ($row = mysqli_fetch_array($result)) { ?>
                                    <option value="<?php echo $row['idLoai']; ?>" <?php if (isset($_POST['cmbCL']) && $_POST['cmbCL'] == $row['idLoai']) {
                                                                                        echo 'selected';
